...
testSuite("NrPercentComplete", suiteTimeout, {
	"Test Case - Calculate Percent Complete for NeuroRestorative User": function(client, done) {
	return client
	...
	.getText("#progressTable tbody tr:nth-child(3) td:nth-child(1)")
	.then(function (saAcq) {
		assert.equal("Service Began via Acquisition Company (as of 2016)?",saAcq);	
	})
	.getText("#progressTable tbody tr:nth-child(4) td:nth-child(1)")
	.then(function (edInv) {
		assert.equal("Educational Involvement at Start of Service",edInv);	
	})
	.getText("#progressTable tbody tr:nth-child(5) td:nth-child(1)")
	...
	.getText("#progressTable tbody tr:nth-child(6) td:nth-child(1)")
	.then(function (ranScore) {
		assert.equal("Rancho Score at Start of Service",ranScore);	
	})
	.getText("#progressTable tbody tr:nth-child(7) td:nth-child(1)")
	.then(function (pcDiag) {
		assert.equal("At least one Diagnosis",pcDiag);	
