...
testSuite("RWNVReferral", suiteTimeout, {
  "should create a Redwood NV Referral successfully": function(client, done) {
	var user = users["RW_NVREF"];
	var d=new Date();
...
//		})
        ...
        .setValue("input[id$=addlocationstate]", "NV")
	    ...
	    .chooseMultiSelectOption("Services Requested", ["DAY - Day Program"])
		...
		.then(function(liftTrans) {
			assert.deepEqual(["0", "1", "2", "3"], liftTrans);
		})		
        ...
        .then(function(funSource) {
         assert.deepEqual(["", "CSS", "Desert Regional Center", "Rural Regional Center",  
                                "Self Pay"], funSource);
         })
	    .chooseSelectOption("Funding Source", "Self Pay")
