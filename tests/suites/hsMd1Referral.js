...
testSuite("HsMd1Referral", suiteTimeout, {
  "should create a Cambridge MD Referral successfully": function(client, done) {
    ...
    return client
         .execUtil("create_referral", {operatingGroup: "Cambridge",flavor: "MD"})
...
//		})
		...
		.fillInputText("CHESSIE ID", "CHESSIE ID Test")
		...
		.then(function(ServicesRequested) {
			assert.deepEqual(["0", "1", "2", "3", "5","6", "7", "8", "9", "10", "11", "12", "13"], ServicesRequested);
		})		
        ...
        .then(function(funSource) {
         assert.deepEqual(["", "CFSA", "DDA", "DJS", "DSS", 
                               "HHS", "Medicaid", "OMHC", "Schools"], funSource);
         })
