trigger EmailForInactiveTMNUsers on TMN_User__c (before insert, before update) {
    ...
    if (Trigger.isUpdate) {
         ...
         */
        if (UserInfo.getLastName() <> 'integration' && u.last_day__c <> Trigger.oldMap.get(u.id).last_day__c) {
         ...
         */
        if (UserInfo.getLastName() == 'integration' && u.last_day__c <> Trigger.oldMap.get(u.id).last_day__c && u.Termination_Date_Modified_Salesforce__c == true) {
